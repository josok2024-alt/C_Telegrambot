...
def _help_text() -> str:
    return (
        ...
        "/settings — view current stake/signal settings\n"
...
async def _handle_command(client: httpx.AsyncClient, chat_id: str, text: str):
    ...
    if command == "/status":
        ...
        await telegram_bot.send_message(
            ...
            f"Min confidence: {config.MIN_CONFIDENCE}% (fixed, edit config.py to change)\n"
            f"Contract duration: {config.TRADE_DURATION_MINUTES}min (fixed, edit config.py to change)\n\n"
            "To change stake or signal count, use:\n"
            "/setstake &lt;amount&gt;\n"
            "/setsignals &lt;count&gt;"
        )
